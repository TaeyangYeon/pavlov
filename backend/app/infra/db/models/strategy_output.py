...
class StrategyOutput(Base):
    ...
    action: Mapped[ActionEnum] = mapped_column(
        SQLAlchemyEnum(ActionEnum, name="action_enum"), nullable=False
    )

    # Take profit levels as JSONB
    # Format: [{"pct": 5.0, "sell_ratio": 0.5}, {"pct": 10.0, "sell_ratio": 0.5}]
    take_profit_levels: Mapped[list[dict[str, Any]]] = mapped_column(
        JSONB, nullable=False, default=list
    )

    # Stop loss levels as JSONB
    # Format: [{"pct": -3.0, "sell_ratio": 1.0}]
    stop_loss_levels: Mapped[list[dict[str, Any]]] = mapped_column(
        JSONB, nullable=False, default=list
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<StrategyOutput(id={self.id}, analysis_log_id={self.analysis_log_id}, "

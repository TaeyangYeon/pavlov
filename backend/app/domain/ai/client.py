...
from app.domain.ai.schemas import AIPromptOutput, TakeProfitLevel, StopLossLevel, StockStrategy
...
class MockAIClient(AIClient):
    ...
    async def call(self, prompt: str) -> AIPromptOutput:
        ...
        return AIPromptOutput(
            market_summary="Mock market analysis: Overall market shows mixed signals with moderate volatility.",
            strategies=[
                StockStrategy(
                    ticker="AAPL",
                    action="hold",
                    ...
                    rationale="Mock strategy: Technical indicators suggest consolidation phase.",
                    confidence=0.7
                )
            ]
        )

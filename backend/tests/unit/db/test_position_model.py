...
def test_position_creation():
    ...
    position = Position(
        user_id=user_id, ticker="AAPL", market="US", entries=entries, status="open"
    )
...
def test_position_entries_is_json():
    ...
    position = Position(user_id=user_id, ticker="GOOGL", market="US", entries=entries)
...
def test_position_market_enum_values():
    ...
    # Valid values
    position_us = Position(user_id=user_id, ticker="AAPL", market="US", entries=entries)
    assert position_us.market == "US"

    position_kr = Position(
        user_id=user_id, ticker="005930", market="KR", entries=entries
    )
    ...
    position_invalid = Position(
        user_id=user_id, ticker="INVALID", market="INVALID_MARKET", entries=entries
    )
...
def test_position_status_enum_values():
    ...
    position_closed = Position(
        user_id=user_id, ticker="AAPL", market="US", entries=entries, status="closed"
    )
    assert position_closed.status == "closed"

    # Test default value (database default, not set in unit test)
    position_default = Position(
        user_id=user_id, ticker="AAPL", market="US", entries=entries
    )
